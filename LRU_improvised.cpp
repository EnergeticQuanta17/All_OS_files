...
void print_pages(vector<int>& f, int iteration_number, int max_cap){
    if(f.size()==0)
        return;
cout<<"After "<<iteration_number<<" pages invoked:";
    cout<<endl;
...
int main(){
    ...
    for(int i=0; i<n; i++){
        ...
        cout<<"Inside pri:\n";
        for(int kk=0;kk<pri.size();kk++)
            cout<<pri[kk]<<" ";
        cout<<endl;
        print_pages(pri, i+1, capacity);
